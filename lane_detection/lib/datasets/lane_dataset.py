...
from .jst import JST
from .lane_dets import LaneDets
...
class LaneDataset(Dataset):
    def __init__(self,
        ...
        if dataset == 'tusimple':
            ...
            self.dataset = JST(split=split, **kwargs)
        elif dataset == 'llamas':
        ...
        elif dataset == 'lane_dets':
            self.dataset = LaneDets(split=split, **kwargs)
        else:
            raise NotImplementedError()

        self.transform_annotations() #useless maybe
    ...
    def find_lane_min_max(self, lane):
        min_y = lane[0][1]
        max_y = lane[0][1]
        for temp in lane:
            if min_y>temp[1]:
    ...
    # for each image or each item in annotation
    def transform_annotation(self, anno, img_wh=None):
        ...
        # anno['lanes'] is stored per image --> map function: call transform_annotation for each time inside anno
        old_lanes = anno['lanes']
        categories = anno['categories'] if 'categories' in anno else [1] * len(old_lanes)
        # old_lanes and categories have the same length
        old_lanes = zip(old_lanes, categories) # make a tuple group by the shortest list
        ...
        lanes[:, 0] = 0
        #print(old_lanes)
        old_lanes = sorted(old_lanes, key=lambda x: x[0][0][0]) # lanes per image along x order: 0->1280
        #print(old_lanes)
        for lane_pos, (lane, category) in enumerate(old_lanes):
            #lower, upper = lane[0][1], lane[-1][1] # y start and y end in for first lane
            lower,upper = self.find_lane_min_max(lane) # lower -> 0, upper -> 720
            xs = np.array([p[0] for p in lane]) / img_w
            ys = np.array([p[1] for p in lane]) / img_h
            lanes[lane_pos, 0] = category
            lanes[lane_pos, 1] = lower / img_h #y: lane start for each lane, larger value
            lanes[lane_pos, 2] = upper / img_h #y: lane end for each lane, smaller value
            lanes[lane_pos, 3:3 + len(xs)] = xs #all x position for each lane
            lanes[lane_pos, (3 + self.dataset.max_points):(3 + self.dataset.max_points + len(ys))] = ys #all y position for each lane
        new_anno = {
            'path': anno['path'],
            'label': lanes,
            'old_anno': anno,
    ...
    def transform_annotations(self):
        ...
        # annotations in jst.py has been updated
    ...
    def __getitem__(self, idx, transform=True):
        ...
        label = item['label']
        if transform:
            line_strings = self.lane_to_linestrings(item['old_anno']['lanes']) # get from origin, to prevent the lane shape be resized twice
            line_strings = LineStringsOnImage(line_strings, shape=img.shape) # lane and image resolution 1280*720
            img, line_strings = self.transform(image=img, line_strings=line_strings) # lane and image resolution 640*360
            #print(img.shape, line_strings.shape)
            line_strings.clip_out_of_image_()
            new_anno = {'path': item['path'], 'lanes': self.linestrings_to_lanes(line_strings)}
            new_anno['categories'] = item['categories'] #categories has been fed into 'label'
