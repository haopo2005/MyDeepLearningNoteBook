...
class PolyRegression(nn.Module):
    ...
    def decode(self, all_outputs, batch_size, conf_threshold=0.5):
        ...
        outputs = outputs.reshape(-1, 10, 9)  # score + upper + lower + 6 coeffs = 9
        #print('outputs size after:',outputs.shape)
    ...
    def loss(self,
        ...
        pred = pred.reshape(-1, target.shape[1], 1 + 2 + 6) # target: 3 dimesion, target.shape[1]->max_lanes
        ...
        target_points, pred_polys = target[:, :, 3:].reshape((-1, target.shape[2] - 3)), pred[:, :, 3:].reshape(-1, 6) #6 poly coeffs
        ...
        pred_xs = pred_polys[:, 0] * ys**5 + pred_polys[:, 1] * ys**4 + pred_polys[:, 2] * ys**3 \
                    + pred_polys[:, 3] * ys**2 + pred_polys[:, 4] * ys + pred_polys[:, 5]
        pred_xs.t_()
