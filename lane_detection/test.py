...
def test(model, test_loader, evaluator, exp_root, cfg, view, epoch, max_batches=None, verbose=True):
    ...
    if epoch > 0:
        ...
        model.load_state_dict(torch.load(os.path.join('/home/jns2szh/code/PolyLaneNet-master/experiments/', "model_{:03d}.pt".format(epoch)))['model'])
    ...
    with torch.no_grad():
        for idx, (images, labels, img_idxs) in enumerate(test_loader):
            ...
            outputs = model.decode(outputs, labels.shape[0], **test_parameters)
            ...
            # cv2.waitKey(0)
            cv2.imwrite('experiments/tusimple_fulltrain/img/%s.png'%(idx), preds)
