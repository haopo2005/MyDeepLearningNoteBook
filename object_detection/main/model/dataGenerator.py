...
def read_image_and_gt(img_files, gt_files, config):
    ...
    def load_annotation(gt_file):
        ...
        for line in lines:
            ...
            #cls = config.CLASS_TO_IDX[obj[0].strip()]
            #print cls
            cls = config.CLASS_TO_IDX['head']
            #get coordinates
            xmin = float(obj[0])
            ymin = float(obj[1])
            xmax = float(obj[2]) + xmin
            ymax = float(obj[3]) + ymin
...
def generator_from_data_path(img_names, gt_names, config, return_filenames=False, shuffle=True ):
    """
   ...
   """
    ...
    if shuffle:
        #permutate images
        print('the images and labels list has been shuffled')
...
def visualization_generator_from_data_path(img_names, gt_names, config, return_filenames=False, shuffle=True ):
    """
